...
RUN test -f src/main/resources/db/migration/V2__add_trip_distance_km.sql \
    ...
    && jar tf target/demo-0.0.1-SNAPSHOT.jar | grep -q 'BOOT-INF/classes/db/migration/V2__add_trip_distance_km.sql' \
    && jar tf target/demo-0.0.1-SNAPSHOT.jar | grep -q 'BOOT-INF/classes/db/migration/V3__add_app_users.sql'
